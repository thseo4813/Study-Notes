...
1. **L4 vs L7**: L4는 IP/Port 기반의 빠른 패킷 전달(단순 분류), L7은 HTTP 내용을 분석한 지능적 라우팅(내용 확인)을 담당한다.
2. **알고리즘 선택**: 서버 스펙이 같으면 **Round Robin**, 다르면 **Weighted**, 연결 시간이 길면 **Least Connections**를 사용한다.
3. **세션 유지**: Stateful 서비스에는 **Sticky Session**이나 **Redis** 같은 공유 저장소가 필수적이다.
4. **헬스 체크**: **Active Check**를 통해 주기적으로 서버 상태를 감시하고, 장애 발생 시 자동으로 트래픽을 차단해야 한다.
