...
```
HTTP  = 엽서를 보내는 것
         → 배달원, 이웃, 누구나 내용을 볼 수 있음
         
HTTPS = 자물쇠 달린 상자에 넣어 보내는 것
         → 열쇠가 없으면 내용을 볼 수 없음
```

### 1.2 실제로 일어나는 일: 패킷 스니핑

HTTP로 로그인하면 무슨 일이 벌어지는가?

```
[당신의 브라우저] ---"ID: admin, PW: 1234"--→ [서버]
                            ↑
                    [공공 WiFi의 해커]
                    "어? ID랑 비밀번호가 그대로 보이네?"
```

**핵심 통찰:** HTTP는 데이터를 **평문(Plain Text)**으로 전송한다. 네트워크 경로 어디서든 누구나 내용을 읽을 수 있다.

### 1.3 HTTPS가 해결하는 것

| 위협 | HTTP | HTTPS |
|-----|------|-------|
| **도청(Eavesdropping)** | 내용이 그대로 노출 | 암호화되어 해독 불가 |
| **변조(Tampering)** | 중간에서 내용 수정 가능 | 무결성 검증으로 변조 감지 |
| **위장(Impersonation)** | 가짜 서버인지 구분 불가 | 인증서로 신원 확인 |
...
```
[개념]
하나의 키로 암호화도 하고, 복호화도 한다.

[비유]
집 열쇠와 같다. 문을 잠글 때와 열 때 같은 열쇠를 쓴다.

[장점] 매우 빠르다 (수학적으로 단순한 연산)
[단점] 열쇠를 어떻게 상대방에게 전달하지?
       → 열쇠를 보내는 순간 누가 훔쳐보면 끝!
```
...
[핵심 원리]
  ✓ 공개키로 잠근 것 → 개인키로만 열 수 있음
  ✓ 개인키로 잠근 것 → 공개키로만 열 수 있음
```
...
1. 서버가 "우체통(공개키)"을 길거리에 설치한다
   → 누구나 이 우체통에 편지를 넣을 수 있음
   
2. 하지만 우체통을 열 수 있는 건 서버만 가지고 있는 "열쇠(개인키)" 뿐
   → 아무리 우체통을 봐도 안에 뭐가 있는지 알 수 없음

3. 당신이 비밀 메시지를 우체통에 넣으면
   → 서버만 열어볼 수 있다
```

**장점:** 키 교환 문제 해결! 공개키는 도청당해도 상관없다.
**단점:** 계산이 복잡해서 **느리다** (대칭키보다 100~1000배 느림)
...
[해결책: 각각의 장점만 취하자]
1. 비대칭키로 "대칭키"를 안전하게 전달 (느려도 한 번만)
2. 이후 실제 통신은 "대칭키"로 빠르게

[비유]
- 비대칭키: 금고 열쇠를 안전하게 전달하는 용도
- 대칭키: 실제로 데이터를 빠르게 주고받는 용도
```
...
#### Phase 1: 왜 협상(Hello)이 필요한가?

```
[문제] 
클라이언트와 서버가 지원하는 암호화 방식이 다를 수 있다.
- 오래된 브라우저: TLS 1.0만 지원
- 최신 서버: TLS 1.3만 지원
→ 통신 불가!

[해결]
서로 "나 이런 거 할 수 있어"라고 말하고, 공통으로 가능한 것 중 
가장 안전한 방식을 선택한다.

[비유]
외국인과 만났을 때 "영어 할 줄 알아? 일본어는? 중국어는?"
→ 서로 통하는 언어를 찾는 과정
```

#### Phase 2: 왜 인증서 검증이 필요한가?

```
[문제]
내가 접속한 서버가 진짜 네이버인지, 가짜 피싱 사이트인지 어떻게 알지?

[해결: 인증 기관(CA)의 역할]
1. 서버는 "나는 네이버야"라는 인증서를 제출
2. 이 인증서는 신뢰할 수 있는 기관(CA)이 서명한 것
3. 브라우저는 내장된 CA 목록과 대조해서 진위 확인
...
위조된 여권인지 진짜인지 정부 기관의 서명으로 판별한다.
```

#### Phase 3: 왜 비대칭키로 "임시 비밀값"을 보내는가?

```
[핵심 통찰]
직접 세션 키를 보내면 안 되는 이유:
→ 나중에 서버 개인키가 유출되면, 과거 통신도 모두 해독됨!

[해결: Forward Secrecy (전방향 안전성)]
임시로 생성한 값(Pre-Master Secret)을 교환하고,
양쪽이 이 값으로 각자 세션 키를 "계산"해서 만든다.

이렇게 하면:
- 세션 키 자체는 네트워크를 타지 않음
- 개인키가 유출되어도 과거 세션 키는 복구 불가
```

#### Phase 4-5: 왜 대칭키(세션 키)로 통신하는가?

```
[이유: 성능]
비대칭키 암호화는 수학적으로 복잡하다 (큰 소수의 곱, 타원곡선 연산 등)
→ 모든 데이터를 비대칭키로 암호화하면 너무 느림

[해결]
안전하게 교환된 대칭키(세션 키)로 실제 데이터를 빠르게 주고받는다.

[결과]
- Handshake: 느리지만 한 번만 (보통 0.1~0.3초)
- 이후 통신: 대칭키로 빠르게 (거의 지연 없음)
```
...
```
[문제: 공개키의 신뢰성]
서버가 "이게 내 공개키야"라고 보내면, 그걸 믿어도 되는가?

해커가 중간에서 가로채고:
"아니야, 이게 진짜 네이버 공개키야" (실제로는 해커 공개키)
→ Man-in-the-Middle Attack!

[해결: 제3자의 보증]
신뢰할 수 있는 기관(CA)이 "이 공개키는 진짜 네이버 것이 맞아"라고 
서명(디지털 서명)해서 보증한다.
```
...
│ 발급자: DigiCert CA         │
│ ─────────────────────────── │
│ [CA의 디지털 서명]          │  ← CA의 개인키로 서명됨
└─────────────────────────────┘

[검증 과정]
1. 브라우저가 인증서 받음
2. 발급자(DigiCert)의 공개키로 서명 검증
3. DigiCert 공개키는 어디서? → 브라우저에 내장된 "신뢰할 수 있는 CA 목록"
4. 서명이 유효하면 → 이 공개키는 진짜 네이버 것!
```
...
[왜 이런 구조인가?]
- Root CA의 개인키는 너무 중요 → 오프라인 금고에 보관
- 일상적인 인증서 발급은 Intermediate CA가 담당
- Root가 해킹당하면 모든 인증서가 무효화되므로, 위험 분산
```
...
```
[TCP의 설계 시대]
1970년대: 네트워크는 불안정, 패킷 손실 흔함
→ "확실하게 전달"이 최우선 목표
→ 속도보다 신뢰성 중시

[현대의 문제]
인터넷은 빨라졌지만, TCP의 "확실하게" 메커니즘이 오히려 병목
```
...
```
[상황]
하나의 TCP 연결로 여러 HTTP 요청을 보냄 (HTTP/2 멀티플렉싱)

[문제]
패킷 1, 2, 3을 보냈는데 패킷 2가 손실됨

TCP의 반응:
"패킷 2가 안 왔어! 패킷 3이 먼저 왔어도 
 패킷 2가 올 때까지 전부 대기!"
...
**서로 독립적인 요청인데, 하나의 문제가 전체를 멈추게 함**

### 6.2 QUIC의 해결책: UDP 위에 새로 설계

```
[발상의 전환]
TCP를 고치는 건 불가능 (전 세계 모든 장비 업데이트 필요)
...
```
[기존 TCP+TLS]
연결마다: SYN → SYN-ACK → ACK → ClientHello → ... (2-3 RTT)

[QUIC]
처음 연결: 1 RTT (Handshake와 데이터를 한 번에)
재연결: 0 RTT (이전에 저장한 키로 바로 데이터 전송!)

시간 →
─────────────────────────────────────────
[QUIC 재연결]
Client: "지난번에 쓰던 키로 바로 데이터 보낼게!"
        [암호화된 데이터] ─────→ Server
...
각 요청이 독립적인 "스트림"으로 분리됨:

스트림 1 (이미지): [패킷1] [패킷2-손실] [패킷3]
                           ↓
                   스트림 1만 재전송 대기
                   
스트림 2 (CSS):    [패킷4] [패킷5] [패킷6] ← 영향 없이 진행!
스트림 3 (JS):     [패킷7] [패킷8] [패킷9] ← 영향 없이 진행!
...
WiFi → 모바일 데이터로 전환하면?
→ IP 주소가 바뀜
→ TCP 연결 끊김 (새로 Handshake 필요)

[QUIC의 해결]
연결을 IP 주소가 아닌 "Connection ID"로 식별
→ IP가 바뀌어도 같은 Connection ID면 연결 유지!

[실생활 예시]
지하철 → 버스로 갈아탈 때도 유튜브 영상이 끊기지 않음
```
...
```
[기존 구조]
TCP (전송) + TLS (보안) = 별개의 계층

[QUIC 구조]
QUIC = 전송 + 보안이 하나로 통합

장점:
- 더 빠른 Handshake (합쳐서 최적화)
- 헤더까지 암호화 (더 강한 보안)
- 중간 장비가 내용을 볼 수 없음
```
...
HTTP/2: 멀티플렉싱 (여러 요청 동시에)
   │
   ↓ 문제: TCP의 Head-of-Line Blocking
   
HTTP/3: QUIC 기반 (스트림 독립성)
   │
   ↓ 현재 최선의 해결책
```
...
| 필드 | 값 예시 | 왜 필요한가? |
|-----|--------|-------------|
| **Content Type** | 0x16 (Handshake), 0x17 (Application Data) | 이 패킷이 뭔지 알아야 처리 방법을 결정 |
...
0x15 = Alert             → "에러 발생!" 또는 "연결 종료"
0x16 = Handshake         → "인사/협상 중"
0x17 = Application Data  → "실제 암호화된 데이터"
```

**실무 팁:** Wireshark에서 `0x15 Alert`가 보이면 Handshake 실패 원인을 찾아야 한다.
...
| Cipher Suite | 특징 | 권장 여부 |
|-------------|------|----------|
| TLS_AES_256_GCM_SHA384 | TLS 1.3 전용, 가장 안전 | ✅ 권장 |
| TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256 | TLS 1.2, 호환성 좋음 | ✅ 권장 |
| TLS_RSA_WITH_AES_128_CBC_SHA | Forward Secrecy 없음 | ❌ 비권장 |
| TLS_RSA_WITH_3DES_EDE_CBC_SHA | 취약한 암호화 | ❌ 사용 금지 |
...
원인: 서버 인증서를 신뢰할 수 없음
해결: CA 인증서 설치 또는 인증서 체인 확인

[Alert Level: Fatal, Description: Handshake Failure]
원인: Cipher Suite 협상 실패
해결: 서버/클라이언트 지원 Cipher Suite 확인
...
[Let's Encrypt (무료)]
- 장점: 무료, 자동 갱신
- 단점: DV(Domain Validation)만 지원
- 적합: 개인 프로젝트, 스타트업

[유료 인증서]
- 장점: OV/EV 인증, 보증금, 기술 지원
- 단점: 비용 (연 $50-$500+)
- 적합: 금융, 대기업, 법적 요구사항 있는 경우

[인증 레벨 이해]
DV: 도메인 소유만 확인 (자동화)
OV: 조직 실재 확인 (서류 검토)
EV: 엄격한 신원 확인 (주소창에 회사명 표시)
```
