...
## 2️⃣ SYN Queue와 Accept Queue 관리
...
| 증상 | 원인 | 해결책 |
|------|------|--------|
| `Cannot assign requested address` | 포트 고갈 | `tcp_tw_reuse=1` + 커넥션 풀 |
| `Connection refused` | Accept Queue 가득 | `somaxconn` + `backlog` ↑ |
| 연결 후 응답 없음 | 좀비 커넥션 | Keep-Alive + Heartbeat |
| `Recv-Q` 계속 증가 | 처리 지연 | I/O/Worker 분리 |
| `Too many open files` | FD 제한 | `ulimit -n` + `fs.file-max` ↑ |
...
**📚 관련 문서:**
- [TCP vs UDP - 소켓 프로그래밍 실무 패턴](./tcp-vs-udp/README.md)
- [프로세스 vs 스레드 - File Descriptor 제한](../01-os/process-vs-thread/README.md)
