# 🧠 가상 메모리: 왜 4GB RAM에 100GB 프로그램이 돌아갈까?

## 💾 실제로 겪어본 메모리 관리 문제들

### 시스템 개발자들이 흔히 마주치는 고민:

**"왜 물리 메모리가 16GB인데 프로그램이 32GB를 사용하지?"**
- 메모리 사용량이 물리 RAM을 초과하는데 왜 안 죽지?
- swap 파일이 계속 커지면서 디스크 I/O가 느려짐
- OutOfMemory는 왜 발생하고 메모리는 왜 부족하지?

**"페이지 폴트가 왜 성능을 죽이나?"**
- 프로그램이 갑자기 느려지면서 CPU 사용률이 낮아짐
- 메모리 사용량은 정상인데 하드디스크가 계속 바쁨
- 캐시 미스가 왜 메모리 관리와 관련이 있지?

**"메모리 단편화는 어떻게 해결하지?"**
- 프로그램 실행 중 메모리가 점점 느려짐
- 가용 메모리는 많은데 할당이 안 됨
- 메모리 컴팩션이 왜 필요하지?

## 🎯 1분 요약: 가상 메모리의 핵심

**가상 메모리 = 실제 메모리 + 하드디스크 스왑**

- **가상 주소**: 프로그램이 보는 주소 (0 ~ 4GB)
- **물리 주소**: 실제 RAM 주소 (0 ~ 16GB)
- **페이지 교체**: 메모리가 부족할 때 디스크로 내보냄
- **장점**: 물리 메모리보다 큰 프로그램 실행 가능

> **결론:**
> 1. **추상화**: 각 프로세스 독립 주소 공간 제공
> 2. **효율화**: 페이지 교체로 메모리 활용 극대화
> 3. **보호**: 프로세스 간 메모리 침범 방지

---

## 2. 🚨 가상 메모리가 해결하는 실제 문제들

### 2.1 물리 메모리의 한계 (현대에도 해당)

**현대 개발 환경의 문제:**
```bash
# 4GB RAM 시스템에서
free -h
# total: 4GB, used: 3.8GB, available: 200MB

# 그런데 8GB 프로그램 실행?
./large-app  # 실행됨! (가상 메모리 덕분)
```

**🚨 실제 문제 사례:**

**문제 1: 메모리 부족으로 인한 페이지 폴트 폭풍**
```java
// 대용량 데이터 처리
List<byte[]> data = new ArrayList<>();
for (int i = 0; i < 1000; i++) {
    data.add(new byte[1024 * 1024]);  // 1MB씩 할당
}
// 물리 메모리 부족 → 디스크 스왑 → 갑자기 느려짐!
```

**문제 2: 메모리 단편화로 인한 할당 실패**
```cpp
// C++에서 메모리 단편화
void* ptr1 = malloc(100);  // 할당
void* ptr2 = malloc(100);  // 할당
free(ptr1);               // 해제 (100바이트 구멍 생김)
// 이후 200바이트 할당 시도 → 실패! (100+100이어도 연속되지 않음)
```

**문제 3: 스레싱(Thrashing)으로 인한 시스템 마비**
```bash
# 메모리 부족으로 페이지 폴트가 너무 많이 발생
# CPU는 페이지 교체하느라 실제 작업을 못 함
# 시스템이 완전히 멈춤 (스레싱 현상)
top  # CPU 90% 사용, 하지만 실제 작업은 안 함
```
...
- **페이지 교체**: LRU, FIFO 등 효율적 알고리즘
- **메모리 보호**: 각 프로세스 독립 공간 보장
