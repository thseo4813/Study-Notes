...
네트워크 전송 계층(TCP/UDP)까지 정리했습니다.

이제 "소프트웨어가 실행되는 환경"을 다루는 **DevOps/인프라** 영역으로 넘어가는 것이 자연스럽습니다. 특히, **"개발 환경과 배포 환경이 달라서 생기는 문제"**를 해결하는 기술부터 시작해 볼까요?

1. **Docker & Container:** 가상 머신(VM)과 컨테이너의 차이, 도커가 혁명인 이유.
